...
def ensemble_photometry(configs, verbose=False):
    ...
    channels = ast.literal_eval(configs['channels'])
    ...
    for channel in channels:
        ...
        try:
            ...
            if verbose:
                print(f"Photometry result: {result['total_counts']}")
                print(f"Photometric error (sigma): {result['sigma']}")
        except Exception as e:
            print(f"Error in aperture photometry: {e}")
            return

        one_shot_file = 'results/all_one_shot.csv'
        save_one_shot(one_shot_file, configs['name'], result['total_counts'], result['sigma'], channel)
...
def main(plot_enabled , config_file='configs/config_sample.csv', verbose=False):

    configs = get_configs(filename=config_file)
    mag_path = configs['magnetars_list_path_file'][0]
    ...
    for i in range(len(mag_list['name'])):        
        mag_config = {
            'psf_file_path': configs['prf_path'][0],
            'name': mag_list['name'][i],
            'image_file_path': configs['data_path_folder'][0]+'/'+mag_list['name'][i],
            'channels': configs['channels'][0],
            'ap_radius': configs['rap_(cam_pix)'][0],
            'inner_ann_radius': configs['rbackin_(cam_pix)'][0],
            'outer_ann_radius': configs['rbackout_(cam_pix)'][0],
            'x_coord': mag_list['ra'][i],
            'y_coord': mag_list['dec'][i],
            'spacing': configs['spacing'][0],
            'grid': configs['grid'][0],
            'intermediate_path_file' : configs['intermediate_path_file'][0]
        }
    ...
    process_all_magnetars(configs['intermediate_path_file'][0], configs['result_path_file'][0])

    
if __name__ == "__main__":
    argparse = argparse.ArgumentParser(description="Simulate PSF placement and perform aperture photometry.")
    argparse.add_argument('-i', '--config', type=str, default="configs/sample_config.coldat", help="config file with circapphot parameters")
